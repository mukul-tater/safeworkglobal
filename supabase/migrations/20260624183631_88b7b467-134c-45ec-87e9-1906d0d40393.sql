...
BEGIN
  ...
  -- 1) Partners (4) — approved x3, applied x1
  FOR rec IN
    SELECT * FROM (VALUES
  ...
  LOOP
    ...
    INSERT INTO public.partner_profiles (
      ...
      status, current_step, partner_code, tier, mobile_verified,
      ...
      rec.status, 6, public.generate_partner_code(), 'bronze', true,
      now() - interval '7 days',
      CASE WHEN rec.status = 'approved' THEN now() - interval '5 days' ELSE NULL END,
      CASE WHEN rec.status = 'approved' THEN auth.uid() ELSE NULL END,
      true, true, true
  ...
  -- 2) Employers (12) in native currencies
  FOR rec IN
    SELECT * FROM (VALUES
  ...
  LOOP
    ...
    -- 3) 2 jobs per employer in native currency
    FOR i IN 1..2 LOOP
  ...
  -- 4) Workers (30); workers 16..30 are sourced via partners
  FOR i IN 1..30 LOOP
  ...
  -- 5) Applications — valid statuses only (PENDING, SHORTLISTED, REVIEWING, INTERVIEWED, APPROVED, OFFERED, HIRED, REJECTED)
