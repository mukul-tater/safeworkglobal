-- Phone OTP is already verified during E-Mitra / worker signup.
-- Confirm those auth users so signInWithPassword can open a session
-- even when the project has "Confirm email" enabled.
-- Also restore partner role assignment dropped from handle_new_user.
