-- Complete worker assessment payment after Razorpay signature verification (edge function).
-- Only callable with service_role (not granted to authenticated clients).
...
-- Edge function uses service_role; do not expose to anon/authenticated.
