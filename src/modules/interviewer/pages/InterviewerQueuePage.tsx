...
export default function InterviewerQueuePage() {
  ...
  const body = (
    <div className={isAdmin ? 'space-y-3 max-w-5xl' : 'max-w-5xl mx-auto px-4 py-6 space-y-3'}>
      {isAdmin && (
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold font-heading">Conduct interviews</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Join the meeting, score the worker, and record the decision. Schedule from Worker queue if none are listed.
            </p>
          </div>
      ...
      {loading ? (
        ...
        <Card className="p-10 text-center text-muted-foreground">
          {isAdmin
            ? 'No interviews scheduled yet. Open Worker queue → Interviews and assign yourself.'
