...
export async function getWorkerDeclarations(userId: string): Promise<WorkerPreJourneyDeclaration | null> {
  ...
  // 1. Check local async storage first
  try {
    const localData = await AsyncStorage.getItem(`${LOCAL_STORAGE_KEY_PREFIX}${userId}`);
    if (localData) {
  ...
  try {
    ...
    if (!error && data) {
      ...
      try {
        await AsyncStorage.setItem(`${LOCAL_STORAGE_KEY_PREFIX}${userId}`, JSON.stringify(decl));
      } catch {
        /* ignore */
      }
...
export async function saveWorkerDeclarations(
  ...
  try {
    await AsyncStorage.setItem(`${LOCAL_STORAGE_KEY_PREFIX}${userId}`, JSON.stringify(declRecord));
  } catch {
    /* ignore */
  }
