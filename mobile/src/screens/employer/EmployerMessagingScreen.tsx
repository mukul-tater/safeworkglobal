...
function MessagingInbox({ title }: { title: string }) {
  ...
  return (
    <ScreenLayout variant="stack">
      <FlatList
        ...
        ListHeaderComponent={<SectionTitle title={title} subtitle={`${items.length} messages`} />}
        ListEmptyComponent={<EmptyState title="No messages yet" subtitle="New conversations will appear here." />}
        renderItem={({ item }) => {
          const inbound = item.receiver_id === profile?.id;
          return (
            <Card>
              <View style={styles.row}>
                <Text style={styles.title}>{item.subject || (inbound ? 'Incoming' : 'Sent')}</Text>
...
export default function EmployerMessagingScreen() {
  return <MessagingInbox title="Messages" />;
}
